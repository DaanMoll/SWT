...
function updateTopNavLinks(params, cruxOverrides) {
    var links = $("li.crux_resource").find("a");
    appendCategory(params, links, cruxOverrides);
	
	// now the support resources which have a different query string format
    var suplinks = $("li.supplemental_crux_resource").find("a");
    appendCategory(params, suplinks, cruxOverrides);
}

function appendCategory(params, links, cruxOverrides) {
    links.each(function(i,link) {
       ...
       href += href.indexOf('?') > 0 ? '&' : '?';
       href += qs;
       // Inclusion of slash in [\w\/] is because category might be physmod/simscape/index,
       // not just matlab/index. (g2205779)
	   href = href.replace(/(category:[\w\/]+\/)index&category=/,"$1");
