var releaseNotesType = 'lucene'; // 'saxonjs' or 'lucene'
if (window.location.href.startsWith('file:')) {
    // For Polyspace Help Browser
    releaseNotesType = 'lucene';
    }
...
function getProductFilteredReleaseNotes(mystylesheetLocation, shortname, allRnReleases, doccentertype, langcode, relpathtohelptop, product_list_location, rncatkey, rncatlabel) {
    ...
    requestHelpService({}, services,
        function(data) {
            // Release Notes with product filter
        ...
        function() {
            // Release Notes without product filter
            SaxonJS.transform({
              stylesheetLocation: mystylesheetLocation,
              sourceLocation: mysourceLocation,
              stylesheetParams: {
    ...
    function postSaxonJSoperations() {
        ...
        $("#doc_survey").replaceWith("<iframe id='doc_survey'/>");
        initDocSurvey();
    }
...
function init() {
    ...
    if (releaseNotesType === 'lucene') {
        ...
        var survey_iframe = '<iframe id="doc_survey"/>';
        $("#doc_survey").replaceWith(survey_iframe);
        initDocSurvey();
    } else {
        handleReleaseNotes();
        showCompatReportDocLink();
