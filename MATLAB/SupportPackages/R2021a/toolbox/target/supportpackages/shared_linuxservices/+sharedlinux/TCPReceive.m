classdef (StrictDefaults)TCPReceive < matlab.System & matlab.system.mixin.Propagates & ...
    ...
    methods (Access = protected)
        ...
        function st = getSampleTimeImpl(obj)
          if isequal(obj.SampleTime_, -1) || isequal(obj.SampleTime_, [-1, 0])
            st = createSampleTime(obj, 'Type', 'Inherited');
          elseif isequal(obj.SampleTime_, [0, 1])
            st = createSampleTime(obj, 'Type', 'Fixed In Minor Step');
          else
            if numel(obj.SampleTime_) == 1
              sampleTime = obj.SampleTime_;
              offset = 0;
            else
              sampleTime = obj.SampleTime_(1);
              offset = obj.SampleTime_(2);
            end
